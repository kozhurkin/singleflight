...
Это небольшая Go‑библиотека для дедупликации конкурентных запросов с опциональным in‑memory‑кешем (TTL) и поддержкой прогрева (warm‑up).
...
По идее это тот же <a href="https://pkg.go.dev/golang.org/x/sync/singleflight" target="_blank" rel="noreferrer">`golang.org/x/sync/singleflight`</a>, предназначенный для дедупликации запросов, но с добавлением механизма кеширования.
...
Если нужно дедуплицировать и кешировать запросы **между процессами**,
в репозитории есть пакет [`github.com/kozhurkin/singleflight/redis`](https://github.com/kozhurkin/singleflight/tree/main/redis), который реализует паттерн поверх Redis.
