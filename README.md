...
**singleflight** — это небольшая Go‑библиотека для дедупликации запросов с дополнительным уровнем удобства по сравнению со стандартным `golang.org/x/sync/singleflight`:
...
Если несколько горутин почти одновременно запросят один и тот же ресурс (один и тот же пользователь, один и тот же отчёт, один и тот же город погоды), без координации вы получите:

- **лишнюю нагрузку** на БД/внешние сервисы;
- **дублирующие, но бесполезные запросы**;
- потенциальные **rate‑limit / throttling ошибки**.
...
- на API‑шлюзах и backend‑сервисах;
- при работе с медленными/платными внешними API;
- для «фан‑аут» сценариев, когда много горутин одновременно идут за одинаковыми данными.

---

## Чем эта реализация лучше стандартной
...
- **Назначение**: дедупликация и (опционально) кеширование вычислений по ключу `K` с результатом типа `V`.
...
### 4. Использование в HTTP‑сервисе (пример из `cmd/example`)
...
Даже при большом количестве параллельных запросов к `/weather?city=spb` внешний API вызывается:

- один раз на «первую волну» запросов;
- затем раз в `cacheTime` (с учётом прогрева).
