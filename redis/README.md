...
- **`lockTTL time.Duration`**
  ...
  - Ограничивает, как долго один инстанс держит блокировку на вычисление результата:
    если владелец блокировки «умер», другие инстансы смогут перехватить вычисление после истечения `lockTTL`.
...
- **`opts ...Option[V]`** — дополнительные опции:
  - **`WithPrefix(prefix string)`**
    ...
    - Удобно для разделения Redis между приложениями / окружениями и избежания коллизий ключей.
  ...
  - **`WithLocalDeduplication(enabled bool)`**
    ...
    - По умолчанию локальная дедупликация выключена.

  - **`WithWarmupWindow(warmupWindow time.Duration)`**
    - Включает «окно прогрева» результата.
    - В интервале `resultTTL` результат для ключа всегда просто берётся из кеша.
    - После этого, в течение дополнительного окна прогрева `warmupWindow`:
      - первый запрос по ключу асинхронно запускает пересчёт и обновление результата в Redis,
      - пока выполняется пересчёт, этот и все последующие запросы по ключу получают **старое** значение из кеша,
      - по окончании пересчёта для ключа обновляется результат в кеше и все последующие запросы получают новое значение.
    - По умолчанию прогрев отключён `warmupWindow == 0`.
